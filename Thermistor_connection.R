...
## event handler is expected to have one argument - received temperature (numeric vector of length 1) -
## and to return a list(send_signal,is_stable,temp) where 
## list$send_signal is a boolean vector of unit length which value is TRUE when we want to send message to server
## list$is_stable is a boolean vector of length 1 which value is TRUE when the process becomes stable and FALSE when it becomes non-stable
...
Connect <- function(host, port, login, pwd, stream_name, event_handler, catch_handler_errors=TRUE) {
    ...
    repeat {
        raw_msg <- ReceiveRawMsg(con);
        event_msg <- Thermistor.Event$read(raw_msg);
        # check error field
        ...
        # apply handler to received temperature
        lst <- NULL;
        tryCatch({
            lst <- event_handler(event_msg$therm);
            # assert lst lengths and types
            stopifnot( is.logical(lst$send_signal), length(lst$send_signal) == 1, !is.na(lst$send_signal),
                       is.logical(lst$is_stable), length(lst$is_stable) == 1, !lst$send_signal || !is.na(lst$is_stable),
                       is.numeric(lst$temp), length(lst$temp) == 1, !lst$send_signal || !is.nan(lst$temp) 
                       );
        }, error = function(e) {
            if (catch_handler_errors) {
                ...
                current_problem_n <<- current_problem_n + 1;
                result$problems[current_problem_n,] <<- list(Sys.time(), problem);
                lst <<- list(send_signal=FALSE, is_stable=NA, temp=NaN);
            }
            else {
                stop(e);
            }
        ...
        if ( lst$send_signal ) {
            message("Handler returned send_signal=TRUE, sending is_stable=", lst$is_stable, ", stable_temp=", lst$temp , " to server!");
            signal_msg <- Thermistor.Signal$new(signal = lst$is_stable, 
                                                therm = lst$temp);
            # send signal message
...
message("Thermistor_connection.R sourced!");
